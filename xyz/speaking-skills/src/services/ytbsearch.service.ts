...
export class YtbSearchService {
  ...
  getTopVideos() {
    var myQuery = 'karaoke english song';
    let stringParamsYoutube= "?key="+this.key+"&type=video&q="+myQuery+"&part=id,snippet&maxResults=20&videoDuration=short&order=viewCount";
    let url = `https://www.googleapis.com/youtube/v3/search` + stringParamsYoutube;
    return new Promise(resolve => {
      this.http.get(url).subscribe(res => {
        //console.log(res);
